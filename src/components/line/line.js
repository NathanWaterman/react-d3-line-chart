...
class Line extends React.Component {
  ...
  updateChart() {
    ...
    for (var i = 0; i < data.length; i++) {
      newData.push([new Date(data[i].date).getDate() + i, data[i].close]);
    }
    ...
    var newXScale = d3.scaleTime().range([0, width]).domain(xDomain);
    var newYScale = d3.scaleLinear().range([height, 0]).domain(yDomain);
    ...
    select(this.ref.current).append('rect')
      .attr('class', 'overlay')
      .attr('width', width)
      .attr('height', height)
      ...
      .on('mousemove', function () {
        ...
        var mouseDate = newXScale.invert(mouse[0]);
        ...
        focus.select('#focusLineX')
          .attr('x1', x).attr('y1', newYScale(yDomain[0]))
          .attr('x2', x).attr('y2', newYScale(yDomain[1]));
        focus.select('#focusLineY')
          .attr('x1', newXScale(xDomain[0])).attr('y1', y)
          .attr('x2', newXScale(xDomain[1])).attr('y2', y);
      });
